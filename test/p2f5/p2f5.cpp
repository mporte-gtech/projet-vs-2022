...
/*
Ton code semble globalement correct pour demander un mot de 5 lettres à l'utilisateur, mais l'erreur std::length_error suggère qu'il y a un problème de gestion de chaîne (probablement une tentative d’accès ou de manipulation de la string incorrectement allouée ou modifiée).

Analysons et corrigeons ton code. Voici les éléments suspects, ligne par ligne :
...
1. ❌ wordLength non défini

Tu utilises wordLength sans l’avoir défini. Cela provoque un comportement indéfini. Si tu compiles sans l’avoir initialisé, c'est probablement la vraie cause du crash (même si ce n’est pas une length_error à proprement parler, cela peut provoquer des erreurs mémoire plus tard).
...
2. 🟨 Retour de fonction mal placé

Tu fais un return stringInput; en dehors de toute fonction (main() ou autre). C++ ne permet pas de retourner une valeur si tu n’es pas dans une fonction.

✅ Solution :

Encapsule ton code dans main() ou une fonction qui retourne un std::string :
...
3. 🟨 Commentaire d’un test sur les caractères

Ce bloc est commenté, donc il ne cause pas l'erreur, mais tu pourrais le réactiver pour filtrer les entrées contenant des caractères non alphabétiques.

Mais attention à une erreur logique ici :

if (toupper(stringInput[i]) == tolower(stringInput[i]))


Cette condition vérifie si un caractère ne change pas entre majuscule et minuscule — ce qui n’est pas une méthode fiable pour savoir si c’est une lettre (par exemple, les chiffres ou symboles aussi peuvent ne pas changer).

✅ Meilleure méthode : utiliser isalpha() :
...
int main() {
	...
	do {
		...
		if (stringInput.length() == wordLength) {
			...
			std::cout << "Le mot doit contenir exactement " << wordLength << " lettres." << std::endl;
		}

	} while (!validWord);

	std::cout << "Mot accepté : " << stringInput << std::endl;
	return 0;
}
...
-> test1
Le mot ne doit contenir que des lettres.
Entrez un mot de 5 lettres
-> salut
Mot accepté : salut


Si tu as encore l’erreur std::length_error, il est probable que tu modifies ailleurs dans ton vrai code la chaîne avec des indices invalides ou des appels incorrects comme .resize() avec une valeur négative ou très grande.

N’hésite pas à me coller tout le fichier .cpp si tu veux une vérification complète.
